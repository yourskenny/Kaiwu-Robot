...
class Agent(BaseAgent):
    def __init__(self, agent_type="player", device=None, logger=None, monitor=None):
        ...
        self._last_loaded_model_file = None
    ...
    def load_model(self, path=None, id="1"):
        ...
        """
        model_file_path = f"{path}/model.ckpt-{id}.pkl"
        # 训练期同一 checkpoint 会被频繁请求；路径未变化时直接复用内存中的模型。
        if model_file_path == self._last_loaded_model_file:
            return False
        self.model.load_state_dict(torch.load(model_file_path, map_location=self.device))
        self._last_loaded_model_file = model_file_path
